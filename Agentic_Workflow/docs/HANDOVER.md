...
For ONNX GPU (optional fastembed path): `pip install onnxruntime-gpu`
...
python scripts/ingest_rag_full.py --import-dtc-pdf "C:\path\to\dtc_list.pdf" --reset --skip-download
```
...
```powershell
python scripts/run_workflow_trace.py
...
- [ ] Clone repo, create `.venv310`, `pip install -r requirements.txt`
- [ ] Copy `.env.example` → `.env` (LLM + Tavily + HF tokens)
- [ ] Download HF dataset **or** run `ingest_rag_full.py --reset`
- [ ] Run `python -c "from src.rag.knowledge_base import knowledge_base; print(knowledge_base.get_stats())"`
- [ ] Run `pytest tests/ -q --ignore=tests/test_keys.py -m "not live"`
- [ ] Run `python scripts/run_workflow_trace.py` once; review `output/workflow_trace_*.md`
- [ ] Run `python -m benchmarks.run_benchmark --mode lite`
...
# --- Benchmark ---
python -m benchmarks.run_benchmark --mode lite
```

---

*Last updated: May 2026 — matches hybrid RAG + HF dataset `aziz9788/automotive-rag-kb`.*
