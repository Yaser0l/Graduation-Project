...
std::vector<WifiAPlist_t> WiFiMulti::getAPlist(void)
{
    return APlist;
}
...
bool WiFiMulti::addAP(const char *ssid, const char *passphrase)
{
    ...
    if (identity)
    { // Allow empty identity as not all WPA2-Enterprise networks require it
        ...
        else
        {
            ...
            for (int8_t i = 0; i < scanResult; ++i)
            {
                ...
                for (uint32_t x = 0; x < APlist.size(); x++)
                {
                    ...
                    if (ssid_scan == entry.ssid || hidden_scan)
                    { // SSID match or hidden network found
                        if (!hidden_scan)
                        {
                        ...
                        if (!entry.hasFailed)
                        {
                            ...
                            { // best network
#if CONFIG_ESP_WIFI_ENTERPRISE_SUPPORT
                                if (sec_scan == WIFI_AUTH_OPEN || ((sec_scan != WIFI_AUTH_WPA2_ENTERPRISE) && entry.passphrase) || ((sec_scan == WIFI_AUTH_WPA2_ENTERPRISE) && entry.passphrase && entry.username && entry.identity))
                                {
#else
