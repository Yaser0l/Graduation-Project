...
static void decode_prius_frame_from_isr(const twai_frame_t *rx_frame)
{
    ...
    case TOYOTA_PRIUS_2010_PT_SPEED_FRAME_ID:
    {
        ...
        if (toyota_prius_2010_pt_speed_unpack(&speed, rx_frame->buffer, rx_frame->buffer_len) == 0)
        {
    ...
    case TOYOTA_PRIUS_2010_PT_WHEEL_SPEEDS_FRAME_ID:
    {
        ...
        if (toyota_prius_2010_pt_wheel_speeds_unpack(&ws, rx_frame->buffer, rx_frame->buffer_len) == 0)
        {
            s_signals.wheel_speed_fl_mph =
                (float)toyota_prius_2010_pt_wheel_speeds_wheel_speed_fl_decode(ws.wheel_speed_fl);
            s_signals.wheel_speed_fr_mph =
                (float)toyota_prius_2010_pt_wheel_speeds_wheel_speed_fr_decode(ws.wheel_speed_fr);
            s_signals.wheel_speed_rl_mph =
                (float)toyota_prius_2010_pt_wheel_speeds_wheel_speed_rl_decode(ws.wheel_speed_rl);
            s_signals.wheel_speed_rr_mph =
                (float)toyota_prius_2010_pt_wheel_speeds_wheel_speed_rr_decode(ws.wheel_speed_rr);
        }
        break;
    }
    ...
    case TOYOTA_PRIUS_2010_PT_STEER_ANGLE_SENSOR_FRAME_ID:
    {
        ...
        if (toyota_prius_2010_pt_steer_angle_sensor_unpack(&steer, rx_frame->buffer, rx_frame->buffer_len) == 0)
        {
            s_signals.steer_angle_deg =
                (float)toyota_prius_2010_pt_steer_angle_sensor_steer_angle_decode(steer.steer_angle);
            s_signals.steer_rate_deg_s =
                (float)toyota_prius_2010_pt_steer_angle_sensor_steer_rate_decode(steer.steer_rate);
        }
        break;
    }
    ...
    case TOYOTA_PRIUS_2010_PT_POWERTRAIN_FRAME_ID:
    {
        ...
        if (toyota_prius_2010_pt_powertrain_unpack(&powertrain, rx_frame->buffer, rx_frame->buffer_len) == 0)
        {
            s_signals.engine_rpm =
                (float)toyota_prius_2010_pt_powertrain_engine_rpm_decode(powertrain.engine_rpm);
        }
        break;
    }
    ...
    case TOYOTA_PRIUS_2010_PT_GAS_PEDAL_FRAME_ID:
    {
        ...
        if (toyota_prius_2010_pt_gas_pedal_unpack(&gas, rx_frame->buffer, rx_frame->buffer_len) == 0)
        {
            s_signals.gas_pedal =
    ...
    case TOYOTA_PRIUS_2010_PT_BRAKE_FRAME_ID:
    {
        ...
        if (toyota_prius_2010_pt_brake_unpack(&brake, rx_frame->buffer, rx_frame->buffer_len) == 0)
        {
    ...
    case TOYOTA_PRIUS_2010_PT_GEAR_PACKET_FRAME_ID:
    {
        ...
        if (toyota_prius_2010_pt_gear_packet_unpack(&gear, rx_frame->buffer, rx_frame->buffer_len) == 0)
        {
...
esp_err_t canmodule_init(void)
{
    ...
    ESP_LOGI(TAG, "TWAI initialized and DBC decoding enabled");
    return ESP_OK;
}
