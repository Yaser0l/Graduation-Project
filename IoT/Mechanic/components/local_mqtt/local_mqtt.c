...
static esp_err_t mqtt_build_topic_locked(const char *suffix, char *out,
                                         size_t out_len) {
  ...
  int topic_len =
      snprintf(out, out_len, "%s%s%s", MQTT_TOPIC_PREFIX, id, suffix);
...
static void mqtt_publish_task_step(void) {
  ...
  if (should_publish && client) {
    ...
    snprintf(
        ...
        "{\"device_id\":\"esp32_s3_1\",\"ts_ms\":%lld,\"rx_frames\":%lu,"
        "\"vehicle_speed_mph\":%.3f,\"wheel_speed_fl_mph\":%.3f,\"wheel_speed_"
        "fr_mph\":%.3f,\"wheel_speed_rl_mph\":%.3f,\"wheel_speed_rr_mph\":%.3f,"
        "\"steer_angle_deg\":%.3f,\"steer_rate_deg_s\":%.3f,\"engine_rpm\":%."
        "3f,\"gas_pedal\":%.3f,\"brake_pedal\":%.3f,\"gear\":%u}",
        (long long)timestamp_ms, (unsigned long)signals.rx_frames,
        signals.vehicle_speed_mph, signals.wheel_speed_fl_mph,
        signals.wheel_speed_fr_mph, signals.wheel_speed_rl_mph,
        signals.wheel_speed_rr_mph, signals.steer_angle_deg,
