...
static void send_json(httpd_req_t *req, int status_code,
...
static bool read_form_value(const char *body, const char *key, char *out,
                            size_t out_len) {
  char pattern[32];
...
static esp_err_t ap_adder_handler(httpd_req_t *req) {
  ...
  if (!wifi_store_add_or_update(s_ap_store, ssid, passphrase,
                                &updated_existing)) {
    send_json(req, 400,
              "{\"status\":\"error\",\"message\":\"AP list is full\"}");
    return ESP_OK;
  }

  if (wifi_store_persist(s_ap_store) != ESP_OK) {
    send_json(req, 500,
              "{\"status\":\"error\",\"message\":\"Failed to save AP\"}");
    return ESP_OK;
  }
...
static esp_err_t ap_reset_handler(httpd_req_t *req) {
  ...
  if (wifi_store_persist(s_ap_store) != ESP_OK) {
    send_json(req, 500,
              "{\"status\":\"error\",\"message\":\"Failed to reset AP list\"}");
    return ESP_OK;
  }
...
static esp_err_t scan_endpoint_handler(httpd_req_t *req) {
  ...
  if (err != ESP_OK) {
    ...
    send_json(
        req, 500,
        "{\"status\":\"error\",\"message\":\"Failed to read scan results\"}");
    return ESP_OK;
  }
  ...
  for (uint16_t i = 0; i < ap_count; ++i) {
    if (records[i].ssid[0] != '\0') {
      cJSON_AddItemToArray(arr,
                           cJSON_CreateString((const char *)records[i].ssid));
    }
  }
...
static esp_err_t mqtt_broker_handler(httpd_req_t *req) {
  ...
  if (broker_uri[0] == '\0') {
    send_json(req, 400,
              "{\"status\":\"error\",\"message\":\"broker_uri is required\"}");
    return ESP_OK;
  }
  ...
  if (mqtt_module_set_broker_uri(broker_uri) != ESP_OK) {
    send_json(
        req, 500,
        "{\"status\":\"error\",\"message\":\"Failed to save broker URI\"}");
    return ESP_OK;
  }
...
static esp_err_t mqtt_broker_get_handler(httpd_req_t *req) {
  ...
  if (!mqtt_module_get_broker_uri(broker_uri, sizeof(broker_uri))) {
    send_json(
        req, 500,
        "{\"status\":\"error\",\"message\":\"Failed to load broker URI\"}");
    return ESP_OK;
  }
