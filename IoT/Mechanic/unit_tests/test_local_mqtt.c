...
static void set_network_up(bool up) {
  esp_event_post(NETWORK_EVENT, up ? NETWORK_EVENT_UP : NETWORK_EVENT_DOWN,
                 NULL, 0, 0);
}

void setUp(void) { mqtt_test_reset(); }
...
void test_mqtt_build_topic_rejects_invalid_args(void) {
  ...
  TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG,
                    mqtt_build_topic_locked(NULL, topic, sizeof(topic)));
  TEST_ASSERT_EQUAL(
      ESP_ERR_INVALID_ARG,
      mqtt_build_topic_locked(MQTT_TOPIC_DATA_SUFFIX, NULL, sizeof(topic)));
  TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG,
                    mqtt_build_topic_locked(MQTT_TOPIC_DATA_SUFFIX, topic, 0));
}
...
void test_mqtt_build_topic_rejects_truncated_buffer(void) {
  ...
  TEST_ASSERT_EQUAL(
      ...
      mqtt_build_topic_locked(MQTT_TOPIC_DATA_SUFFIX, topic, sizeof(topic)));
}
...
void test_mqtt_set_broker_uri_restarts_on_change(void) {
  ...
  TEST_ASSERT_EQUAL_STRING("mqtt://new-broker",
                           mqtt_client_stub_get_last_uri());
}
...
void test_mqtt_set_broker_uri_does_not_restart_on_same_value(void) {
  ...
  TEST_ASSERT_EQUAL(ESP_OK,
                    mqtt_module_set_broker_uri(MQTT_DEFAULT_BROKER_URI));
...
void test_mqtt_publish_dtc_requires_client(void) {
  ...
  TEST_ASSERT_EQUAL(ESP_ERR_INVALID_STATE,
                    mqtt_module_publish_dtc("VIN", "payload"));
}
...
void test_mqtt_publish_dtc_reports_publish_failure(void) {
  ...
  TEST_ASSERT_EQUAL(ESP_ERR_INVALID_STATE,
                    mqtt_module_publish_dtc("VIN123", "payload"));
}
...
void test_mqtt_publish_task_step_publishes_data(void) {
  ...
  TEST_ASSERT_NOT_NULL(
      strstr(mqtt_client_stub_get_last_payload(), "\"ts_ms\":1234"));
}
